...
type CallbackRequestDto struct {
	...
	BillCount         int                   `json:"bill_count" validate:"required"`                             // Numbers of bill
	BillDetails       []BillDetailDto       `json:"bill_details,omitempty"`                                     // Details of a given bill
	AdditionalDetails []AdditionalDetailDto `json:"additional_details,omitempty"`                               // Additional details of a given bill that will be shown to the user with a collapsed initial state
}
