...
-- Tabel respons: satu baris per respons dari Telkomsel (SYNC atau CALLBACK)
...
COMMENT ON TABLE transaction.telkomsel_transaction IS 'Tracking lifecycle transaksi dari RabbitMQ ke Telkomsel API';
COMMENT ON TABLE transaction.telkomsel_transaction_response IS 'Log setiap respons dari Telkomsel API (SYNC dan CALLBACK)';
COMMENT ON COLUMN transaction.telkomsel_transaction.status IS 'PROCESSING | SUCCESS | FAILED';
COMMENT ON COLUMN transaction.telkomsel_transaction_response.response_type IS 'SYNC | CALLBACK';
